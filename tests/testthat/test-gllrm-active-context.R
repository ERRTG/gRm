...
test_that("active context stores terms in source position order", {
  ia <- gRm(active_context_data(), items = c("I1", "I2", "I3"), exogenous = "X1", id = "ID")
  spec <- gllrm(ia, ld = ~ I2:I1, dif = ~ I3:X1)
  bundle <- build_item_parameters_bundle(ia$project)

  context <- build_gllrm_active_context(spec, bundle)
...
test_that("active context counts observed LD, DIF, and first-seen score/exogenous groups", {
  ia <- gRm(active_context_data(), items = c("I1", "I2", "I3"), exogenous = "X1", id = "ID")
...
test_that("score/exogenous groups ignore inactive DIF backgrounds in the grouping key", {
  context <- list(
    valid_rows = 1:4,
    n_backgrounds = 2L,
    backgrounds = data.frame(name = c("X_active", "X_inactive")),
  ...
  expect_equal(groups$X_active, c(1L, 2L))
})

test_that("active components preserve source item order and initialize state", {
  ia <- gRm(active_context_data(), items = c("I1", "I2", "I3"), exogenous = "X1", id = "ID")
...
test_that("active context stores matrix fast paths matching data-frame metadata", {
  ia <- gRm(active_context_data(), items = c("I1", "I2", "I3"), exogenous = "X1", id = "ID")
