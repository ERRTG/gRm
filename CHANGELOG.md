...
## gRm 1.2.0 - 2026-09-03

- Aligned the CM2/CM3 object and summary print methods with the package's shared
  diagnostic rounding convention while retaining unrounded numeric result
  objects. Exact empirical bootstrap zero remains `0`, and unsigned bootstrap
  seeds now print exactly even under low `options(digits)`.

- Expanded the installed `cm2()` and `cm3()` help pages into a complete public
  interface reference. The documentation now states every argument's accepted
  values, validation and conditional behavior; distinguishes displayed score
  bounds from source table-count classification of extreme scores; describes
  the source bootstrap capability limits and all three bootstrap result states;
  and documents the stable selection, numerical, summary, and bootstrap result
  structures.
- Added source-faithful focal item selection to `cm2()` and `cm3()` by exact
  fitted item names or one-based indices, with strict normalization, stable
  selection metadata, full-model score/exogenous/LD/DIF semantics, bootstrap
  reuse of the selected margin list, and DIGRAM's zero-degree-of-freedom
  invariance convention.

## gRm 1.1.0 - 2026-08-31

- Renamed the public `m2()` and `m3()` diagnostics to `cm2()` and `cm3()` throughout the package, including result classes, internal schemas, tests, and documentation, to use the correct CM2/CM3 terminology.

## gRm 1.0.0 - 2026-08-17

- Added deterministic parallel CM2/CM3 bootstrap refitting through the public
  `bootstrap_jobs` control. All response samples are still generated first in
  one sequential Delphi-RNG pass; only the RNG-free fit and diagnostic phase
  uses ordered POSIX fork workers (`mc.set.seed = FALSE`), with a serial Windows
  fallback. Results record requested/effective workers and execution mode, and
  generated response matrices remain omitted unless explicitly requested.
  Bootstrap acceptance now explicitly uses the fitter's stopping-step
  `report_delta`, matching Pascal's global `delta < 0.1` test, rather than the
  R-only sufficient-margin discrepancy recomputed after the fit loop. The
  capability guard now also enforces canonical `MaxItems8 = 10` through
  `ReviewComponents`, `MaxnumberOfMulticomp = 4`, and `NsimMax = 15,000`,
  replacing the stale 40-item allowance and previously missing source-array
  bounds. It also rejects any fitted fixed-score component distribution with
  more than 255 positive patterns before Pascal's `npatterns: Byte` boundary
  could wrap. Effective fork workers are capped by detected logical cores as
  well as the number of simulations. Bootstrap refits now apply the wrapper's
  inclusive `0..highest_possible_score` estimation window, ensuring both
  generated extreme-score blocks contribute to `Nvalid` and fitted margins.
- Replaced the CM2/CM3 bootstrap's validation-only Park--Miller and bounded
  whole-test shortcuts with DIGRAM 7.04's executable trajectory: Delphi 4's
  wrapped uint32 `134775813 * RandSeed + 1` stream, `Randomize` time-of-day seed,
  both `x := 1 - random` transforms, component-score traversal, and
  `CollectComprecords` response-pattern order. Reproducible seeds now accept
  the complete uint32 range, and focused tests pin short vectors, a 2,331,644
  draw recovered trajectory, exact generated samples, and per-replicate states.
- Made `source/digram_source_20260817/` the canonical DIGRAM 7.04 reference.
  Source traces now resolve units exactly through `scd/scd.dpr`: local `scd/`
  paths govern the program, forms, dialogs, and orchestration, while explicit
  `..\SKunits\` paths govern computational/statistical units. Same-named files
  are never selected by proximity.
- Updated CM2/CM3 to the canonical 7.04 `Prepare_CM3tests` behavior. Active DIF
  margins are now filtered by the corrected item-first
  `ItemBias(.i1,i2.)` lookup, and separate observed CM2 and CM3
  Benjamini-Hochberg tables retain the source's 85-value cap. Structured
  results retain every prepared margin, matching 7.04's removal of the old
  three-way significance display gates. DIGRAM's interactive bootstrap prompt
  now documents its zero-simulation default; the R API remains opt-in and uses
  100 only after bootstrap is enabled.
- Version-gated the historical DIGRAM 7.02 MULTIEXO active-GLLRM CM3 report.
  It remains inventoried with an explicit reason but is not a parity target
  because its 27-margin output encodes the superseded transposed DIF lookup;
  replacement requires a report generated by the canonical 7.04 binary.
- Before the 7.04 reference promotion, certified the then-current
  7.02-compatible audit-remediation tree against all 197 inventoried oracle
  rows across six projects. That historical durable run parsed 2,415,004 oracle
  ...
  compared. The validation log and coverage snapshot point to
...
- Split the oversized SCREEN J, item-fit, summary/presentation, global-homogeneity, and CM2/CM3-bootstrap modules along source-routine and responsibility boundaries. Numerical kernels, iteration order, and public schemas are unchanged. The former 317-line SCREEN J entry point is decomposed into source-ordered item traversal and schema assembly helpers, and each CM2/CM3 replicate now has an isolated generate/refit/accept/analyze boundary, so orchestration, exact simulation, conditional diagnostics, and rendering are independently auditable.
...
- Reused the vignette's fitted GLLRM for ARI, CM2, and CM3 instead of refitting the identical model three additional times, preserving all demonstrated results while materially reducing build/check time.
...
- Documented the complete CM2/CM3 bootstrap algorithm and source trace, removed stale “experimental” labels from the package README, added the standalone durable parity runner, and reconciled the oracle-coverage document so it no longer contradicts its active CM3 adapter section.
- Added focused CM2/CM3 bootstrap unit coverage for strict public controls, opt-in defaults, private seeded reproducibility, fixed score strata, the source score-resampling no-op, active LD/DIF generation, historical extreme-score exogenous handling, refit acceptance, CM2 result shape, printed summaries, and accepted-sample `simulated p <= observed p` calibration.
- Added durable CM2/CM3 bootstrap trajectory validation that persists raw Pascal
  and R results, diagnostic tables, rendered output, normalized trajectories,
  exact comparisons, and run metadata. The source-faithful package trajectory
  now pins Delphi uint32 states, draw counts, fixed score margins, and response
  matrices; the former Park--Miller harness trajectory is retained only as
...
- Added the source-shaped CM2/CM3 parametric-bootstrap engine required by the
  public CM2/CM3 diagnostics. It preserves observed score distributions within
  ...
  starts a fresh 5,000-step GLLRM refit for every sample, accepts only final
  `delta < 0.1` samples, and counts simulated p-values in Pascal's `<=`
  direction. A private exact Delphi 4 uint32 stream makes seeded runs
  reproducible without mutating R's global RNG, while structured raw
  ...
  and source static bootstrap bounds are reported as capability metadata. All
  admissible models use SKbias8's component-score and within-component pattern
  generator; no distribution-equivalent whole-test shortcut changes its random
  draw count or later trajectory.
...
- Promoted `cm2()` and `cm3()` from their deterministic-only experimental
  ...
  re-estimation, 5,000-step refit limit, response retention, and the preserved
...
- Restored DIGRAM's item-fit FDR flags in the public `item_fit()` test table.
  ...
  directly.
- The `cm2()` and `cm3()` help pages now mark the diagnostics as experimental in
...
- Added source-backed `cm2()` and `cm3()` fit diagnostics for fitted `gRm_fit`
...
- CM2/CM3 item-exogenous margin preparation now follows the preserved DIGRAM
...
- The oracle validation module now parses and compares every numeric value in
  the two checked-in MULTIEXO CM3 reports against the R-side `cm3()` diagnostic,
