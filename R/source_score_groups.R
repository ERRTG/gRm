...
items_select_values <- function(project) {
  ...
  if (n_complete < 1L) {
    stop(
      "Item-selection score groups require at least one complete item response pattern.",
      call. = FALSE
    )
  }
...
item_selection_default_cut <- function(score_counts) {
  ...
  n <- if (length(score_counts) > 2L) {
    sum(score_counts[seq.int(2L, length(score_counts) - 1L)])
  } else {
    0L
  }
  cut <- 0L
